...
export async function POST(req: NextRequest) {
  if (!BACKEND_URL) {
    console.error('[newsletter] NEXT_PUBLIC_API_URL is not set');
  ...
  try {
    ...
    console.error('[newsletter] failed to parse request body', err);
    return NextResponse.json(
  ...
  try {
    ...
    console.error('[newsletter] upstream fetch failed', { target, message });
    return NextResponse.json(
      {
  ...
  try {
    ...
    console.error('[newsletter] upstream returned non-JSON', {
