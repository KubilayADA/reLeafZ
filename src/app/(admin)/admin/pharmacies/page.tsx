...
import { getAdminPharmacies } from '@/lib/adminApi'
...
export default function AdminPharmaciesPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError('')
      try {
        const res = await getAdminPharmacies({
          page,
          limit,
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-black/10">
                ...
                <th className="px-4 md:px-6 py-3 text-left font-medium text-gray-600">Delivery Type</th>
                ...
                : pharmacies.map((p, idx) => (
                    <tr key={p.id ?? idx} className="border-b border-black/5 last:border-b-0 hover:bg-black/[0.02]">
                      <td className="px-4 md:px-6 py-3 text-gray-700">
                        <Link href={`/admin/pharmacies/${p.id}`} className="hover:text-green-700">
                          {p.id ?? '—'}
                        </Link>
                      </td>
